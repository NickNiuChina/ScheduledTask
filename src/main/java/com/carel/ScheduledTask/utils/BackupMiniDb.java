...
public class BackupMiniDb {
	
	private static Logger logger=Logger.getLogger(ScheduledTask.class);
	
	public static int backupMiniDb() {
		try {
			
			Properties props = GetProperties.getProperties();
			
	        // 数据库连接信息
			String dbHost = props.getProperty("dbHost-mini");
            String dbName = props.getProperty("dbName-mini");
            String userName = props.getProperty("userName-mini");
            String password = props.getProperty("password-mini");
            String dbPort = props.getProperty("dbPort-mini");
    		...
    		String bfName = String.format("wechat-mini-prod-back-%s.backup", nowtime);
